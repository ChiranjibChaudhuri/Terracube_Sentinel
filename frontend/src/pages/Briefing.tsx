...
import { Clock, Download, FileText, FileCode2, Shield, Lock } from 'lucide-react'
...
function StructuredSection({ section, index }: { section: BriefingResponse['sections'][number]; index: number }) {
  return (
    ...
    >
      <h3 className="mb-2.5 flex items-center gap-2 text-sm font-bold text-primary">
        <span className="h-1 w-0.5 bg-primary rounded-full" />
        {section.title}
      </h3>
    ...
    <motion.div className="max-w-4xl space-y-6" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Automated threat assessment and operational summary</p>
        </div>
                    ...
                    <div className="mt-2.5 flex flex-wrap items-center gap-5 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1.5">
                        ...
                        <Badge variant="outline" className="gap-1.5 text-emerald-400 border-emerald-400/20 bg-emerald-400/5">
                          ...
                          {briefingQuery.data.classification}
                        </Badge>
                      </span>
                      <span className="flex items-center gap-1.5">
          ...
          {briefingView === 'structured' && briefingQuery.data ? (
            <CardContent className="space-y-6 px-8 py-6">
              ...
              <p className="text-center text-[10px] italic text-muted-foreground">
                TerraCube Sentinel · Automated Intelligence Platform · {new Date().toLocaleDateString()}
              </p>
            </CardContent>
          ) : markdownQuery.data ? (
